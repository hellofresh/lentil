    ...
    compileLibs(options) {
        ...
        if (!this.moduleName) {
            return this.collectAllLibFiles(libsConfig)
                ...
                .then((collectedLibs) => Promise.all(
                    collectedLibs.map((lib) => this.writeDistFile(
        ...
        return FileSystem.concat(libsConfig[this.moduleName])
            .then((contents) => {
                ...
                Logger.debug(`Uglified libs code for ${this.moduleName}:`,
                    uglified);

                return uglified;
            })
            .then((contents) => this.writeDistFile(this.moduleName, 'libs', 'js',
                contents, options.shouldMinify));
    }
